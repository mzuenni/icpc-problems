...
struct exitVerdict {
	const int verdict;
	constexpr exitVerdict(int _verdict = FAIL) : verdict(_verdict) {}
};

std::ostream& operator<<(std::ostream& str, const exitVerdict& t) {
	str << std::endl;
...
private:	
	...
	void reportFail() {
		jout << "input failed" << reportLine() << exitVerdict(failVerdict);
	}
	...
	void isNotWhitespace() {
		if (strict) {
			auto c = this->peek();
			checkFail();
			if (isWhiteSpace(traits_type::to_char_type(c))) jout << "invalid whitespace found" << reportLine() << exitVerdict(failVerdict);
		}
	}
...
public:
	...
	std::string readToken(const std::regex& pattern) {
		...
		if (!std::regex_match(token, pattern)) {
			jout << "Token: '" << token << "' does not match pattern" << reportLine() << exitVerdict(failVerdict);
		}
		return token;
	}
	...
	integer_t readInteger(integer_t l, integer_t r) {
		...
		if (res < l || res >= r) {
			jout << "Integer " << res << " out of range [" << l << ", " << r << ")" << reportLine() << exitVerdict(failVerdict);
		}
		return res;
	}
	...
	real_t readReal(real_t l, real_t r) {
		...
		if (std::isnan(res) || res < l || res >= r) {
			jout << "Real " << res << " out of range [" << l << ", " << r << ")" << reportLine() << exitVerdict(failVerdict);
		}
		return res;
	}
	...
	void readSpace() {
		if (!strict) {
			...
			this->get(c);
			checkFail();
			if (c != ' ') jout << "missing space" << reportLine() << exitVerdict(failVerdict);
		}
	}
	...
	void readEOL() {
		if (!strict) {
			...
			this->get(c);
			checkFail();
			if (c != '\n') jout << "missing EOL" << reportLine() << exitVerdict(failVerdict);
		}
	}
	...
	void readEOF() {
		if (!strict) {
			...
			if (this->peek() != EOF) jout << "missing EOF" << reportLine() << exitVerdict(failVerdict);
		}
	}
	...
	void readDirt() {
		if (strict) {
			...
			if (foundToken) jout << "Dirt found" << exitVerdict(failVerdict);
		}
	}
};
...
namespace mzDetails {
	...
	void checkJerr() {
		if (!jerr.good()) std::cerr << "Could not open error stream" << exitVerdict(FAIL);
	}

	void checkJout() {
		if (!jout.good()) jerr << "Could not open judge output" << exitVerdict(FAIL);
	}

	void checkPout() {
		if (!pout.good()) jerr << "Could not open participants output" << exitVerdict(FAIL);
	}

	void checkTin() {
		if (!tin.good()) jerr << "Could not read test case" << exitVerdict(FAIL);
	}

	void checkJans() {
		if (!jans.good()) jerr << "Could not read jury answer" << exitVerdict(FAIL);
	}

	void checkPans() {
		if (!pans.good()) jerr << "Could not read participants answer" << exitVerdict(FAIL);
	}
}
...
public:
	...
	CommandParser(int argc, char **argv) : raw(argc) {
		...
		for (int i = 0; i < argc; i++) {
			...
			if (mzDetails::startsWith(raw[i], COMMAND_PREFIX)) {
				...
				if (commands.count(raw[i]) != 0) jout << "Duplicated argument: " << raw[i] << exitVerdict(FAIL);
				command = raw[i];
				first = i+1;
				count = 0;
			} else {
	...
	public:
		...
		const std::string& asString() {
			if (token == nullptr) jout << "accessed invalid parameter" << exitVerdict(FAIL);
			return *token;
		}
		...
		integer_t asInteger() {
			if (token == nullptr) jout << "accessed invalid parameter" << exitVerdict(FAIL);
			integer_t res;
			if (!parse(asString(), res)) jout << "parameter is not an integer: " << asString() << exitVerdict(FAIL);
			return res;
		}
		...
		real_t asReal() {
			if (token == nullptr) jout << "accessed invalid parameter" << exitVerdict(FAIL);
			real_t res;
			if (!parse(asString(), res)) jout << "parameter is not a real: " << asString() << exitVerdict(FAIL);
			return res;
		}
...
void init_io(int argc, char **argv) {
	if (argc != 4) std::cerr << "Wrong number of arguments: " << argc << ", expected 4" << exitVerdict(FAIL);
...
void init_gen(int argc, char **argv) {
	...
	if (!mzDetails::mzTestcaseOut.good()) jerr << "Could not open output stream" << exitVerdict(FAIL);
