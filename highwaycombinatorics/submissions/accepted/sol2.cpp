...
//with these parameters every field can be solved
//checked with bruteforce
constexpr ll fibsLim = 32;
constexpr ll baseLim = 2000000;
	...
	for (ll l = 0, r = sz(base); r < baseLim; l = r, r = sz(base)) {
		for (ll i = l; i < r; i++) {
			for (ll j = base[i].fibs.back(); j < sz(fibs); j++) {
				...
				e.ops += fibs[j].ops + 1;
				e.fibs.push_back(j);
				if (e.ops < lim / 2) base.push_back(e);
			}
		}
	}
	sort(all(base));
