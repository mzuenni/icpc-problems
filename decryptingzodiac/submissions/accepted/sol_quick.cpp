...
vector<ll> solve(const string& a, const string& b) {
	...
	for (ll i = 0; i < n; i++) {
		...
		pb[2*sigma*(i+1) - 1 -(b[n-i-1]-'a')].real = 1;
		pb[2*sigma*(i+1) - 1 -(b[n-i-1]-'a') - sigma].real = 1;
	}
	fft(pa);
	fft(pb);
	for (ll i = 0; i < lim; i++) pa[i] = pa[i] * pb[i];
	fft(pa, true);
	vector<ll> res;
	for (ll dx = n-1; dx >= 0; dx--) {
		for (ll dc = sigma - 1; dc < 2*sigma - 1; dc++) {
			res.push_back(round(pa[dx*2*sigma+dc + 2*sigma*n].real));
		}
	}
	return res;
}
