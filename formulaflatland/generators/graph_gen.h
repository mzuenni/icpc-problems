...
namespace GraphDetail {
	...
	template<typename E>
	std::ostream& operator<<(std::ostream& os, const EdgeWrapper<E>& e) {
		os << e.from << " " << e.to;
	...
	class GraphType final {
		constexpr static Integer DEFAULT_MIN_ID = 0;
		...
		inline void assertNode(Integer id) const {
			if (id < minId || id >= minId + nodeCount()) {
	...
	public:
		...
		GraphType(Integer from, Integer to) : 
			adj(to - from),
			minId(from) {
			assert(from <= to);
		}
		// construct empty graph with vertices in [0, n)
		GraphType(Integer n = 0) :
		...
		ConstEdge getEdgeById(size_t id) const {
			if (id >= edges.size()) {
				throw std::out_of_range("Index \"" + std::to_string(id) + "\" out of range [0, " + std::to_string(edges.size()) + ") !");
			}
		...
		Graph& append(const Graph& o) {
			...
			for (const auto& e : o.getEdges()) {
				addEdge(e.from + diff, e.to + diff, *e);
			}
			return *this;
		}
		...
		Graph& permutate() {
			...
			Random::shuffle(edges.begin(), edges.end());
			for (auto& e : edges) {
				if constexpr (!DIR) if (Random::bit()) e.to ^= e.fromXorTo;
				e.fromXorTo ^= e.to;
	...
	template<typename E, bool DIR>
	std::ostream& operator<<(std::ostream& os, const GraphType<E, DIR>& g) {
...
// star with root = 0 and chilrden [1, leafes]
template<typename E = NoData>
Graph<E> star(Integer leafes) {
	return bipartite<E>(1, leafes);
}
...
template<typename E = NoData>
Graph<E> lolly(Integer c, Integer p) {
...
// prefix of the rado graph i->j <=> i<j && j[i] == true;
// vertices in [0, n)
template<typename E = NoData, bool DIR = false>
...
Graph<E> pruefer(const std::vector<Integer>& code) {
	...
	auto getNext = [&](){
		...
		skip = -1;
		if (oldSkip >= 0 && oldSkip <= current) return oldSkip;
...
Graph<E> randomGraph(const std::vector<Integer>& degree, Integer averageFlips = 10) {
	...
	for (Integer i = 0; i < averageFlips*res.nodeCount(); i++) {
		...
		if (a.from == b.to || res.hasEdge(a.from, b.to)) continue;
		if (b.from == a.to || res.hasEdge(b.from, a.to)) continue;
