...
constexpr integer_t k = 5;
constexpr integer_t q = 200000;
constexpr integer_t sigma = 6;
...
int main(int argc, char **argv) {
	...
	for (integer_t i = 0; i < q; i++) {
		integer_t p;
		if (randomBit()) p = randomInteger<integer_t>(max(0ll, sz(state) - 1000), sz(state));
		else p = randomInteger<integer_t>(0ll, min(1000ll, state.size()));
