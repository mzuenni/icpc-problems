...
int main(int argc, char **argv) {
	...
	set<Integer> tmp;
	while (sz(tmp) < lim) tmp.insert(Random::integer(1, range));
	vector<Integer> integers(all(tmp));
	...
	priority_queue<pair<Integer, Integer>, vector<pair<Integer, Integer>>, greater<pair<Integer, Integer>>> todo;
	for (Integer i = 0; i < lim; i++) {
		deg[i] = sz(adj[i]);
		todo.push({deg[i], i});
	}
