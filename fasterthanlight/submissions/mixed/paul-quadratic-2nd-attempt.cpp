...
int main() {
	...
	for (const auto &p: corners) {
		...
		bool found_vertical = false; // did we find any rectangle not touching our point?
		bool found_horizontal = false; // did we find any rectangle not touching our point?
		...
		for (auto [x1,y1,x2,y2]: rectangles) {
			...
			if (p.x < x1) {
				intersect(a, b, found_horizontal);
			} else if (p.y < y1) {
				intersect(c, d, found_vertical);
			} else {
				assert(0);
			}
			if (!ok) break;
